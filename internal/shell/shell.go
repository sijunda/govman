...
func (s *BashShell) SetupCommands(binPath string) []string {
	return []string{
		...
		"        eval \"$(govman use \"$version\" 2>/dev/null | grep '^export ')\"",
		"      fi",
		"    fi",
	"  else",
	"    # No .govman-version file, revert to default version",
	"    echo \"🏠 Switching back to default Go version\"",
	"    eval \"$(govman use default 2>/dev/null | grep '^export ')\"",
	"  fi",
		"}",
		"",
...
func (s *FishShell) SetupCommands(binPath string) []string {
	return []string{
		...
		"        echo \"🐹 Switching to Go $version (from .govman-version)\"",
		"        govman use \"$version\" | source",
		"      end",
		"    end",
		"  else",
		"    # No .govman-version file, revert to default version",
		"    echo \"🏠 Switching back to default Go version\"",
		"    govman use default | source",
		"  end",
		"end",
		"",
		"# Trigger auto-switch for current directory on shell startup",
...
func (s *PowerShell) SetupCommands(binPath string) []string {
	return []string{
		...
		"          & govman use $version | Where-Object { $_ -like '$env:PATH*' } | Out-String | Invoke-Expression",
		"        }",
		"      }",
		"    } catch {",
		...
		"  } else {",
		"    # No .govman-version file, revert to default version",
		"    Write-Host \"🏠 Switching back to default Go version\" -ForegroundColor Green",
		"    & govman use default | Where-Object { $_ -like '$env:PATH*' } | Out-String | Invoke-Expression",
		"  }",
	"}",
		"",
