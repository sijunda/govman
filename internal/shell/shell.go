...
func (s *BashShell) SetupCommands(binPath string) []string {
	return []string{
		...
		"# Only add govman to PATH if there's an active Go version",
		"if [ -L \"" + binPath + "/go\" ]; then",
...
func (s *PowerShell) SetupCommands(binPath string) []string {
	return []string{
		...
		"# Added by govman installer",
		"# Only add govman to PATH if there's an active Go version",
		"if (Test-Path \"" + binPath + "\\go.exe\" -PathType Leaf) {",
		"  " + s.PathCommand(binPath),
		"}",
		"",
		"# Initialize completion",
...
func (s *CmdShell) SetupCommands(binPath string) []string {
	return []string{
		...
		"REM Note: For session-only changes, manually add the Go version to PATH",
		"REM Example: set PATH=C:\\Users\\YourName\\.govman\\versions\\go1.21.5\\bin;%PATH%",
		"REM For persistent changes, add the following line:",
		"REM " + s.PathCommand(binPath),
